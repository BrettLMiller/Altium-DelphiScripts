...
Procedure TotalWipeoutComponentsToSymbols(const CurrentLib : ISch_Lib); forward;
...
//        until FileName = '';
    ...
    dConfirm := ConfirmNoYesWithCaption('Totally Eliminate/Remove ALL models from all CMps in All Schibs (make symbols) ','Very sure ? ');
    if not dconfirm then exit;
...
// without this will process but not save back to filesystem!
        ...
        if SchLib <> nil then
        begin
            TotalWipeoutComponentsToSymbols(SchLib);
...
begin
    ...
    TotalWipeoutComponentsToSymbols(CurrentLib);

    GenerateReport(ModelsList, 'DataFileLinksRemovedFromCompLib.txt');
    Modelslist.Free;
end;

Procedure TotalWipeoutComponentsToSymbols(const CurrentLib : ISch_Lib);
Var
    Iterator           : ISch_Iterator;
    Component          : ISch_Component;
    ImplIterator       : ISch_Iterator;
...
// danger this completely removes the models.
 ...
 //  Refresh library.
    ...
    SetDocumentDirty(true);
    ModelsList.Add('Count of FP Model totally removed/deleted : ' + IntToStr(FLinkCount));
End;
